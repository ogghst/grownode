...
gn_err_t _gn_leaf_start(gn_leaf_handle_intl_t leaf_config) {
	...
	int ret = GN_RET_OK;
	ESP_LOGI(TAG, "starting leaf: '%s', type '%s'", leaf_config->name,
			leaf_config->leaf_descriptor->type);

	TaskHandle_t task_handle;

	char _taskname[GN_LEAF_NAME_SIZE + 7];
	strncpy(_taskname, "task_", 6);
	strncat(_taskname, leaf_config->name, GN_LEAF_NAME_SIZE);

	if (xTaskCreate((void*) leaf_config->leaf_descriptor->callback, _taskname,
			leaf_config->task_size, leaf_config,
			GN_LEAF_TASK_PRIORITY, //configMAX_PRIORITIES - 1,
			&task_handle) != pdPASS) {
		ESP_LOGE(TAG, "failed to create leaf task for %s", leaf_config->name);
		goto fail;
	}
...
void _gn_keepalive_callback(gn_config_handle_intl_t conf) {
	ESP_LOGI(TAG, "keepalive triggered");
...
void _gn_keepalive_stop(gn_config_handle_intl_t conf) {
	...
	ESP_LOGI(TAG, "keepalive task paused");
}

gn_leaf_handle_intl_t _gn_leaf_get_by_name(gn_config_handle_intl_t conf,
		char *leaf_name) {
...
gn_err_t _gn_send_event_to_leaf(gn_leaf_handle_intl_t leaf_config,
	...
	if (leaf_config->node->config->status != GN_NODE_STATUS_STARTED
			&& evt->id != GN_NODE_STARTED_EVENT) {
		ESP_LOGW(TAG_EVENT,
				"_gn_send_event_to_leaf - GN_RET_ERR_NODE_NOT_STARTED - id: %d, param %s, leaf %s, data %.*s",
				evt->id, evt->param_name, evt->leaf_name, evt->data_len,
				evt->data);
		return GN_RET_ERR_NODE_NOT_STARTED;
	}

	ESP_LOGD(TAG_EVENT,
			"_gn_send_event_to_leaf - id: %d, param %s, leaf %s, data '%.*s', len=%d",
			evt->id, evt->param_name, evt->leaf_name, evt->data_len, evt->data,
			evt->data_len);
	...
	if (xQueueSend(leaf_config->event_queue, evt, pdMS_TO_TICKS(1000)) != pdTRUE) {
		ESP_LOGE(TAG,
				"xQueueSend failed - not possible to send message to leaf %s",
				leaf_config->name);
...
void _gn_evt_handler(void *handler_data, esp_event_base_t base, int32_t id,
	...
	case GN_NET_RBT_START:
		if (conf->status == GN_NODE_STATUS_STARTED)
			gn_reboot();
		break;
	...
	case GN_SRV_KEEPALIVE_TRIGGERED_EVENT:
		...
		if (gn_mqtt_send_keepalive(conf->node_handle) != GN_RET_OK) {
	...
	case GN_LEAF_PARAM_CHANGE_REQUEST_EVENT: {
		...
		gn_leaf_handle_intl_t leaf_config = _gn_leaf_get_by_name(conf,
				evt->leaf_name);
...
esp_err_t _gn_init_keepalive_timer(gn_config_handle_intl_t conf) {
	...
	if (conf->config_init_params->server_keepalive_timer_sec == 0) {
		ESP_LOGW(TAG, "keepalive timer not enabled");
		return ESP_OK;
	}
...
esp_event_loop_handle_t gn_leaf_get_event_loop(gn_leaf_handle_t leaf_config) {
	...
	if (!leaf_config)
		return NULL;
	return ((gn_leaf_handle_intl_t) leaf_config)->node->config->event_loop;

}

/**
 * 	@brief wait until a message has received for the specific leaf
 *
 * 	@param leaf	the leaf for which the event should be triggered
 * 	@param evt 	pointer to the event where to put data received
 * 	@param ms_to_wait number of millisec to wait.
 *
 * 	@return GN_RET_OK if message has correctly received
 * 	@return GN_RET_TIMEOUT if no messages is fired after ms_to_wait msec
 * 	@return GN_RET_ERR on other errors
 */
gn_err_t gn_leaf_receive_event(gn_leaf_parameter_event_handle_t evt, gn_leaf_handle_t leaf, int ms_to_wait) {

	if (!leaf || !evt) return GN_RET_ERR;

	if (xQueueReceive(gn_leaf_get_event_queue(leaf), evt,
	portMAX_DELAY) == pdTRUE)
		return GN_RET_OK;
	return GN_RET_TIMEOUT;
...
gn_err_t gn_node_start(gn_node_handle_t node) {
	...
	//init mqtt system
	ESP_GOTO_ON_ERROR(gn_mqtt_start(_node->config), err_srv, TAG,
	...
	ESP_GOTO_ON_ERROR(_gn_init_keepalive_timer(_node->config), err_srv, TAG,
...
gn_leaf_handle_intl_t _gn_leaf_config_create() {

	gn_leaf_handle_intl_t _conf = (gn_leaf_handle_intl_t) malloc(
...
gn_leaf_handle_t gn_leaf_create(gn_node_handle_t node_config, const char *name,
		gn_leaf_config_callback callback, size_t task_size,
		UBaseType_t priority) { //, gn_leaf_display_task_t display_task) {
	...
	gn_leaf_handle_intl_t l_c = _gn_leaf_config_create();
...
gn_leaf_descriptor_handle_t gn_leaf_get_descriptor(gn_leaf_handle_t leaf_config) {
	return ((gn_leaf_handle_intl_t) leaf_config)->leaf_descriptor;
}
...
gn_err_t gn_leaf_get_name(gn_leaf_handle_t leaf_config, char *name) {
	...
	if (!leaf_config)
		return GN_RET_ERR_INVALID_ARG;
	strcpy(name, ((gn_leaf_handle_intl_t) leaf_config)->name);
...
gn_leaf_handle_t gn_leaf_get_config_handle(gn_node_handle_t node,
	...
	for (size_t i = 0; i < _node->leaves.last; i++) {
		...
		gn_leaf_handle_intl_t _lc = (gn_leaf_handle_intl_t) _node->leaves.at[i];
...
gn_node_handle_t gn_leaf_get_node(gn_leaf_handle_t leaf_config) {
	...
	if (!leaf_config)
		return NULL;
	return ((gn_leaf_handle_intl_t) leaf_config)->node;
...
//TODO find a way to hide the freertos structure
	if (!leaf_config)
		return NULL;
	return ((gn_leaf_handle_intl_t) leaf_config)->event_queue;
...
void _gn_leaf_evt_handler(void *handler_args, esp_event_base_t base, int32_t id,
	...
	//gets leaf
	gn_leaf_handle_intl_t leaf_config = (gn_leaf_handle_intl_t) handler_args;
	...
	if (id == GN_LEAF_PARAM_CHANGED_EVENT
		...
		gn_leaf_parameter_event_handle_t evt =
				(gn_leaf_parameter_event_handle_t) malloc(
						sizeof(gn_leaf_parameter_event_t));
...
gn_err_t gn_leaf_event_subscribe(gn_leaf_handle_t leaf_config,
	...
	ESP_LOGD(TAG_EVENT, "gn_leaf_event_subscribe event: %d, leaf %s", event_id,
			((gn_leaf_handle_intl_t ) leaf_config)->name);
...
gn_err_t gn_leaf_event_unsubscribe(gn_leaf_handle_t leaf_config,
	...
	ESP_LOGD(TAG_EVENT, "gn_leaf_event_unsubscribe event: %d, leaf %s",
			event_id, ((gn_leaf_handle_intl_t ) leaf_config)->name);
...
//check if existing
		...
		if (gn_storage_get(_buf, (void**) &value)
				== GN_RET_NVS_PARAMETER_FOUND) {
	...
	//_ret->name = _name;
	strncpy(_ret->name, name, GN_LEAF_PARAM_NAME_SIZE);
	strncpy(_ret->unit, "", GN_LEAF_PARAM_UNIT_SIZE);
	strncpy(_ret->format, "", GN_LEAF_PARAM_FORMAT_SIZE);
...
gn_err_t gn_leaf_param_init_string(const gn_leaf_handle_t leaf_config,
	...
	if (_param->validator) {
		...
		strcpy(_param->param_val->v.s, val);
	}
	...
	esp_err_t ret = esp_event_post_to(_leaf_config->node->config->event_loop,
			GN_BASE_EVENT, evt.id, &evt, sizeof(evt), portMAX_DELAY);
	if (ret != ESP_OK) {
		ESP_LOGE(TAG,
...
gn_err_t gn_leaf_param_force_bool(const gn_leaf_handle_t leaf_config,
	...
	if (!leaf_config || !name) {
		ESP_LOGD(TAG, "gn_leaf_param_force_bool: GN_RET_ERR_INVALID_ARG");
		return GN_RET_ERR_INVALID_ARG;
	}
	...
	ESP_LOGD(TAG,
			"gn_leaf_param_force_bool: param '%s', val '%d', new val '%d'",
	...
	if (_param->validator) {
		...
		gn_leaf_param_validator_result_t val_ret = _param->validator(_param,
				(void**) validate);
		if (val_ret == GN_LEAF_PARAM_VALIDATOR_PASSED
				|| val_ret == GN_LEAF_PARAM_VALIDATOR_PASSED_CHANGED) {
			_param->param_val->v.b = **validate;

			ESP_LOGD(TAG, "gn_leaf_param_force_bool: after validator: %d",
					_param->param_val->v.b);

		} else {
			ESP_LOGD(TAG, "gn_leaf_param_force_bool: validation error. code %d",
					val_ret);
			return GN_RET_ERR_INVALID_ARG;
		}
...
gn_err_t gn_leaf_param_force(const gn_leaf_param_handle_t param_handle,
	...
	if (_param->access == GN_LEAF_PARAM_ACCESS_IMMUTABLE) {
		ESP_LOGW(TAG,
				"gn_leaf_param_force - paramater is immutable, change discarded");
	...
	gn_leaf_handle_intl_t _leaf_config = _param->leaf;
...
gn_err_t gn_leaf_param_init_double(const gn_leaf_handle_t leaf_config,
	...
	strcpy(evt.param_name, _param->name);
	evt.id = GN_LEAF_PARAM_INITIALIZED_EVENT;
	evt.data_len = sprintf(&evt.data[0], "%f", _param->param_val->v.d) + 1;
 ...
 * if the parameter has not GN_LEAF_PARAM_ACCESS_ALL permission, it won't be updated
 * @return ESP_OK if parameter is changed,
 * @return GN_RET_ERR_LEAF_PARAM_ACCESS_VIOLATION if not permitted
 */
...
//check param name
		if (strcmp(param, leaf_params->name) == 0) {
			...
			//check if has write access
			if (leaf_params->access != GN_LEAF_PARAM_ACCESS_ALL) {
				ESP_LOGW(TAG,
...
gn_err_t gn_leaf_param_add_to_leaf(const gn_leaf_handle_t leaf,
	...
	gn_leaf_param_handle_intl_t _param = ((gn_leaf_handle_intl_t) leaf)->params;

	while (_param) {
		if (strcmp(_param->name, new_param->name) == 0) {
			ESP_LOGE(TAG, "Parameter with name %s already exists in Leaf %s",
					new_param->name, ((gn_leaf_handle_intl_t ) leaf)->name);
			return GN_RET_ERR_INVALID_ARG;
		}
		if (_param->next) {
			_param = _param->next;
	...
	if (_param) {
		...
		((gn_leaf_handle_intl_t) leaf)->params = new_param;
	}

	//TODO put in mqtt on connected code
	/*
	 gn_err_t ret = GN_RET_OK;
	...
	ESP_LOGD(TAG, "Param %s added in %s", new_param->name,
			((gn_leaf_handle_intl_t ) leaf)->name);
	return GN_RET_OK;
...
gn_err_t gn_send_node_leaf_param_status(const gn_node_handle_t _node_config) {
	...
	for (int i = 0; i < node_config->leaves.last; i++) {
		...
		gn_leaf_handle_intl_t leaf_config = node_config->leaves.at[i];
...
gn_err_t gn_send_leaf_param_change_message(const char *leaf_name,
	...
	ESP_LOGD(TAG, "gn_send_leaf_param_change_message: param '%s', data='%.*s'",
			param_name, message_len, (char* ) message);
...
gn_err_t gn_leaf_param_set_bool(const gn_leaf_handle_t leaf_config,
	...
	char val_ptr[1];
	val_ptr[0] = (val ? true : false);
	//memcpy(val_ptr, val ? true : false, 1);
...
gn_leaf_param_handle_t gn_leaf_get_params(gn_leaf_handle_t leaf_config) {
	...
	if (!leaf_config)
		return NULL;
	return ((gn_leaf_handle_intl_t) leaf_config)->params;
...
gn_leaf_param_handle_t gn_leaf_param_get_param_handle(
	...
	gn_leaf_param_handle_intl_t param =
			((gn_leaf_handle_intl_t) leaf_config)->params;
...
void* _gn_leaf_context_add_to_leaf(const gn_leaf_handle_t leaf, char *key,
	...
	gn_leaf_handle_intl_t leaf_config = (gn_leaf_handle_intl_t) leaf;
...
gn_err_t gn_storage_set(const char *key, const void *value,
	 ...
	 */
	ESP_LOGD(TAG_NVS, "gn_storage_set(key=%s, pointer value=%s, size=%d)", key,
			(const char* )value, required_size);
...
// Read previously saved blob if available
		*value = calloc(required_size + sizeof(uint32_t), 1);
		...
		ESP_LOGD(TAG_NVS, "gn_storage_get(%s) - %s - OK", key, (char* ) *value);
	} else
		goto fail;
...
// Close
	...
	return required_size == 0 ?
			GN_RET_NVS_PARAMETER_NOT_FOUND : GN_RET_NVS_PARAMETER_FOUND;
