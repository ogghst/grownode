...
void app_main(void) {
	...
	gn_leaf_param_set_double(lights1in, GN_RELAY_PARAM_GPIO, 25);
	gn_leaf_param_set_bool(lights1in, GN_RELAY_PARAM_STATUS, true);

	gn_leaf_config_handle_t lights2in = gn_leaf_create(node, "lights2in",
			gn_relay_config, 4096);
	gn_leaf_param_set_double(lights2in, GN_RELAY_PARAM_GPIO, 33);
	gn_leaf_param_set_bool(lights2in, GN_RELAY_PARAM_STATUS, true);

	gn_leaf_config_handle_t pltonoffin = gn_leaf_create(node, "pltonoffin",
			gn_relay_config, 4096);
	gn_leaf_param_set_double(pltonoffin, GN_RELAY_PARAM_GPIO, 23);
	gn_leaf_param_set_bool(pltonoffin, GN_RELAY_PARAM_STATUS, true);

	gn_leaf_config_handle_t pltin = gn_leaf_create(node, "pltin", gn_relay_config,
			4096);
	gn_leaf_param_set_double(pltin, GN_RELAY_PARAM_GPIO, 5);
	gn_leaf_param_set_bool(pltin, GN_RELAY_PARAM_STATUS, true);

	gn_leaf_config_handle_t waterpumpin = gn_leaf_create(node, "waterpumpin",
			gn_relay_config, 4096);
	gn_leaf_param_set_double(waterpumpin, GN_RELAY_PARAM_GPIO, 17);
	gn_leaf_param_set_bool(waterpumpin, GN_RELAY_PARAM_STATUS, true);

	gn_leaf_config_handle_t waterlevelin = gn_leaf_create(node, "waterlevelin",
			gn_capacitive_water_level_config, 4096);
	gn_leaf_param_set_double(waterlevelin, GN_CWL_PARAM_TOUCH_CHANNEL, 2);
	gn_leaf_param_set_double(waterlevelin, GN_CWL_PARAM_UPDATE_TIME_SEC, 10);

	gn_leaf_config_handle_t hcc_in = gn_leaf_create(node, "hcc_in",
			gn_relay_config, 4096);
	gn_leaf_param_set_double(hcc_in, GN_RELAY_PARAM_GPIO, 26);
	gn_leaf_param_set_bool(hcc_in, GN_RELAY_PARAM_STATUS, false);

	gn_leaf_config_handle_t hcc_speed = gn_leaf_create(node, "hcc_speed",
			gn_pump_config, 4096);
	gn_leaf_param_set_double(hcc_speed, GN_PUMP_PARAM_GPIO, 2);
	gn_leaf_param_set_double(hcc_speed, GN_PUMP_PARAM_POWER, 0);
	gn_leaf_param_set_bool(hcc_speed, GN_PUMP_PARAM_STATUS, false);

	gn_leaf_config_handle_t fan_in = gn_leaf_create(node, "fan_in",
			gn_relay_config, 4096);
	gn_leaf_param_set_double(fan_in, GN_RELAY_PARAM_GPIO, 27);
	gn_leaf_param_set_bool(fan_in, GN_RELAY_PARAM_STATUS, false);

	gn_leaf_config_handle_t fan_speed = gn_leaf_create(node, "fan_speed",
			gn_pump_config, 4096);
	gn_leaf_param_set_double(fan_speed, GN_PUMP_PARAM_GPIO, 14);
	gn_leaf_param_set_double(fan_speed, GN_PUMP_PARAM_POWER, 0);
	gn_leaf_param_set_bool(fan_speed, GN_PUMP_PARAM_STATUS, false);

	gn_leaf_config_handle_t bme280 = gn_leaf_create(node, "bme280",
			gn_bme280_config, 4096);
	gn_leaf_param_set_double(bme280, GN_BME280_PARAM_SDA, 21);
	gn_leaf_param_set_double(bme280, GN_BME280_PARAM_SCL, 22);
	gn_leaf_param_set_bool(bme280, GN_BME280_PARAM_ACTIVE, true);
	gn_leaf_param_set_double(bme280, GN_BME280_PARAM_UPDATE_TIME_SEC, 10);
